...
//+------------------------------------------------------------------+
#ifndef RR_RISKMANAGER_MQH
#define RR_RISKMANAGER_MQH
...
//+------------------------------------------------------------------+
class CRrRiskManager
  {
private:
   SRrSettings         m_cfg;
   CRrSymbolCtx       *m_sym;
   SRrRiskState        m_state;
...
public:
                     CRrRiskManager(void): m_sym(NULL),m_day(0),m_last_scan(0) {}
   ...
   SRrRiskState        State(void) const { return(m_state); }
   bool              IsHalted(void) const { return(m_state.halted); }
  };

//+------------------------------------------------------------------+
bool CRrRiskManager::Init(const SRrSettings &cfg,CRrSymbolCtx *sym)
  {
   m_cfg=cfg;
   m_sym=sym;
   ...
   RLog.Info(StringFormat("Risk: %.2f%% per trade, daily stop %.2f%%, max positions %d, start equity %.2f",
...
//+------------------------------------------------------------------+
void CRrRiskManager::ScanHistory(void)
  {
   ...
   if(!HistorySelect(from,now+60))
     {
      RLog.Warn("HistorySelect failed - risk counters may be stale this tick");
      return;
     }
...
//+------------------------------------------------------------------+
void CRrRiskManager::Refresh(void)
  {
   ...
   if(today!=m_day)
     {
      ...
      m_state.halt_reason      = "";
      m_last_scan              = 0;
      RLog.Info(StringFormat("New trading day - counters reset, equity %.2f",equity));
     }
...
//+------------------------------------------------------------------+
bool CRrRiskManager::TradingAllowed(string &reason,string &tag)
  {
   if(m_state.halted)
     {
   ...
   if(m_cfg.daily_loss_percent>0.0 && m_state.daily_pl_percent<=-m_cfg.daily_loss_percent)
     {
      ...
      RLog.Warn("Halting new entries for today: "+m_state.halt_reason);
      reason=m_state.halt_reason;
   ...
   if(m_cfg.daily_profit_percent>0.0 && m_state.daily_pl_percent>=m_cfg.daily_profit_percent)
     {
      ...
      RLog.Info("Halting new entries for today: "+m_state.halt_reason);
      reason=m_state.halt_reason;
   ...
   if(m_cfg.max_drawdown_percent>0.0 && m_state.drawdown_percent>=m_cfg.max_drawdown_percent)
     {
      ...
      RLog.Warn("Halting new entries: "+m_state.halt_reason);
      reason=m_state.halt_reason;
...
//+------------------------------------------------------------------+
double CRrRiskManager::LossPerLot(const int dir,const double entry,const double stop) const
  {
   double profit=0.0;
...
//+------------------------------------------------------------------+
double CRrRiskManager::CalcLots(const int dir,const double entry,const double stop,
                              const double existing_volume,string &reason,string &tag)
  {
   reason="";
   tag="";
   ...
   if(m_cfg.lot_mode==RR_LOT_FIXED)
