...
public:
   ...
   int               freeze_level;     // points
                     ...
                     CSymbolCtx(void): symbol(""),digits(0),point(0),tick_size(0),tick_value(0),
...
//+------------------------------------------------------------------+
bool CSymbolCtx::Init(const string sym)
  {
   symbol=sym;

   if(!SymbolSelect(symbol,true))
     {
      Log.Error(StringFormat("Symbol %s could not be selected in Market Watch",symbol));
      return(false);
     }
   ...
   if(point<=0.0 || vol_step<=0.0 || vol_min<=0.0)
     {
      Log.Error(StringFormat("Incomplete symbol spec for %s (point=%g volStep=%g volMin=%g)",
   ...
   Log.Info(StringFormat("Symbol %s: digits=%d point=%g volMin=%s volStep=%s stops=%dpt freeze=%dpt",
...
//+------------------------------------------------------------------+
void CSymbolCtx::RefreshLevels(void)
  {
      ...
      for(int i=0;i<n;i++)
        {
         ...
         if(StringSplit(item,'-',edges)!=2 ||
           ...
           {
            Log.Error(StringFormat("Bad time window '%s' - expected HH:MM-HH:MM",item));
            return(false);
           }
...
//+------------------------------------------------------------------+
string TsDirName(const int dir)
  {
   if(dir>0) return("BUY");
...
//+------------------------------------------------------------------+
//--- "PERIOD_M15" -> "M15"
string TsTimeframeName(const ENUM_TIMEFRAMES tf)
  {
